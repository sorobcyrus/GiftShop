...
BEGIN
  ALTER TABLE GiftShop.[Order]
   ...
   CONSTRAINT FK_Order_Employee_OrderEmployeeID FOREIGN KEY (EmployeeID)
      REFERENCES GiftShop.Employee (EmployeeID);
...
SET @ErrorText = 'Failed adding FOREIGN KEY for Table GiftShop.EmployeeTask.';

IF EXISTS (SELECT *
	FROM sys.foreign_keys
	WHERE object_id = OBJECT_ID(N'GiftShop.FK_EmployeeTask_EmployeeEmployeeID')
	AND parent_object_id = OBJECT_ID(N'GiftShop.EmployeeTask')
)
BEGIN
  SET @Message = 'FOREIGN KEY for Table GiftShop.EmployeeTask already exist, skipping....';
  RAISERROR(@Message, 0,1) WITH NOWAIT;
  EXEC GiftShop.InsertHistory @SP = @SP,
   @Status = 'Run',
   @Message = @Message;
...
ELSE
BEGIN
  ALTER TABLE GiftShop.EmployeeTask
   ADD CONSTRAINT FK_EmployeeTask_EmployeeEmployeeID FOREIGN KEY (EmployeeID)
      REFERENCES GiftShop.[Employee] (EmployeeID),
    CONSTRAINT FK_EmployeeTask_TaskID FOREIGN KEY (TaskID)
      REFERENCES GiftShop.[Task] (TaskID);

  SET @Message = 'Completed adding FOREIGN KEY for TABLE GiftShop.EmployeeTask.';
  RAISERROR(@Message, 0,1) WITH NOWAIT;
  EXEC GiftShop.InsertHistory @SP = @SP,
   @Status = 'Run',
   @Message = @Message;
...
SET @ErrorText = 'Failed adding FOREIGN KEY for Table GiftShop.Employee.';

IF EXISTS (SELECT *
   FROM sys.foreign_keys
	WHERE object_id = OBJECT_ID(N'GiftShop.FK_Employee_EmployeeID')
	AND parent_object_id = OBJECT_ID(N'GiftShop.Employee')
)
BEGIN
   SET @Message = 'FOREIGN KEY for Table GiftShop.Employee already exist, skipping....';
   RAISERROR(@Message, 0,1) WITH NOWAIT;
   EXEC GiftShop.InsertHistory @SP = @SP,
   @Status = 'Run',
   @Message = @Message;
...
ELSE
BEGIN
   ALTER TABLE GiftShop.Employee
   ADD CONSTRAINT FK_Employee_Employee_ManagerID FOREIGN KEY (ManagerID)  -- PK is EmployeeID, so ManagerID is FK
      REFERENCES GiftShop.Employee (EmployeeID)

  SET @Message = 'Completed adding FOREIGN KEY for TABLE GiftShop.Employee.';
  RAISERROR(@Message, 0,1) WITH NOWAIT;
  EXEC GiftShop.InsertHistory @SP = @SP,
   @Status = 'Run',
   @Message = @Message;
